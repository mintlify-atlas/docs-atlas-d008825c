---
title: "Get Started with AI Startup Analyzer"
...
<Steps>
  ...
  <Step title="Submit Your First Analysis">
    ...
    **Poll for progress** while the 14 agents are running:
    ...
    The `progress` field moves from `0` → `5` (queued) → `89` (Phase 1 complete) → `98` (Phase 2 complete) → `100` (database written). When `status` is `COMPLETED`, fetch the full report:
