...
description: "Explore the AI Startup Analyzer architecture: NestJS backend, Next.js frontend, BullMQ job queue, Redis cache, Prisma + PostgreSQL, and 14 parallel AI agents."
---
        ...
        └─► AnalysisService.create()  — persists record with status PENDING
              └─► BullMQ "analysis" queue  — job enqueued
                    └─► AnalysisProcessor.process()  — worker picks up job
                          ...
                          ├─► Phase 2: 8 advanced agents (Promise.allSettled)
...
When Phase 1 completes, the job progress is updated to `89`. The combined `agentResults` object is passed as context to every Phase 2 agent.

### Phase 2 — Eight Advanced Agents (parallel, fault-tolerant)
